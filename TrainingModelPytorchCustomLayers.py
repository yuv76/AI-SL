...
if __name__ == "__main__":
    ...
    plt.plot(range(1, Constants.NUM_EPOCHS + 1), valid_accuracies, label="Validation Accuracy", color='r', marker='x')
    plt.scatter(Constants.NUM_EPOCHS, test_accuracy, label="Test Accuracy", color='g', s=100, zorder=5)  # s=100 controls dot size
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy (%)')
    plt.title(f'Training and Validation Accuracy Over Epochs\n Took total time of {total_time} seconds')
    plt.grid(True)
    plt.legend()

    plt.savefig('accuracy_custom.png')
